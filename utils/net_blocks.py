...
## fc layers
def fc(nc_inp, nc_out, batch_norm=True, drop_prob=0.0):
    if batch_norm:
...
def linear(nc_inp, nc_out, bias=True, initializer="kaiming_normal"):
    func = nn.Linear(nc_inp, nc_out, bias=bias)
    if initializer == "kaiming_normal":
        init.kaiming_normal_(func.weight)
    elif initializer == "xavier_uniform":
        init.xavier_uniform_(func.weight)
    elif initializer == "xavier_normal":
        init.xavier_normal_(func.weight)
    return func
